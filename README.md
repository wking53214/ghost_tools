...
- **Mechanical** (`ghost_buster/mechanical.py`) -- deterministic, AST-based,
  ...
  doubt about a deterministic check. Six detectors as of v0.7:
  ...
  *semantic* `doc_drift` detector already names (same `Category.DOC_DRIFT`)
  ...
  `merge_conflict_marker` (v0.7) is the one detector here that deliberately
...
295 tests, 0 network calls, 0 API key required -- the semantic-layer tests
