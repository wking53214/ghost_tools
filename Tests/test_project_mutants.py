...
MUTANTS = [
    ...
    # --- test_config_collects_nothing stops seeing, or starts inventing ---
    ("a testpaths entry that does not exist reads as present", _P,
     "        missing = [entry for entry in paths if not (root / entry).exists()]\n",
     "        missing = []\n"),
    ("a runner pointed at nothing is silent", _P,
     "    if config is None or not config.missing:\n        return []\n",
     "    if True:\n        return []\n"),
    ("a repository with no tests is reported for its plans", _P,
     "    if not report.test_files:\n",
     "    if False:\n"),
    ("collecting nothing is downgraded to the partial-miss severity", _P,
     '            root, "tests nobody can collect", Severity.MAJOR,\n',
     '            root, "tests nobody can collect", Severity.MINOR,\n'),
    ("one stale path of several is inflated to the silent-green severity", _P,
     '        root, "a configured test path is missing", Severity.MINOR,\n',
     '        root, "a configured test path is missing", Severity.MAJOR,\n'),
    ("a partial miss is treated as collecting nothing", _P,
     "        return bool(self.paths) and len(self.missing) == len(self.paths)\n",
     "        return bool(self.missing)\n"),
    ("CI being configured suppresses the runner finding after all", _P,
     "        return findings, report\n",
     "        return [], report\n"),
    ("only pyproject.toml is read, so an ini-configured runner is invisible", _P,
     '_PYTEST_CONFIG_FILES = ("pytest.ini", "pyproject.toml", "tox.ini", "setup.cfg")\n',
     '_PYTEST_CONFIG_FILES = ("pyproject.toml",)\n'),
    ("the test-configuration state stops printing, so checked and unlooked-at read alike", _P,
